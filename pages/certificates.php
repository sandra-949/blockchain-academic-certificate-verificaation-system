...
if ($filter === 'valid')   $where .= " AND c.status='valid'";
if ($filter === 'revoked') $where .= " AND c.status='revoked'";

// Institution users only see their own certificates
...
<div class="page-header d-flex justify-content-between align-items-start flex-wrap gap-2">
    <div>
        ...
        <p>Browse and manage all issued academic certificates.</p>
    </div>
...
<div class="page-card">
    ...
    <table class="table-cv">
        ...
        <?php if ($certs && $certs->num_rows > 0): ?>
            <?php $i = 1; while($cert = $certs->fetch_assoc()): ?>
            <tr>
                <td style="color:var(--text-muted);font-size:0.82rem;"><?php echo $i++; ?></td>
                <td>
                    <strong><?php echo htmlspecialchars($cert['studentName']); ?></strong><br>
                    <span style="font-size:0.78rem;color:var(--text-muted);">ID: <?php echo htmlspecialchars($cert['studentID']); ?></span>
                </td>
                ...
                <td>
                    <div class="hash-display" style="max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"
                ...
                <td>
                    ...
                    <?php if ($userRole === 'admin'): ?>
                        ...
                        <form method="POST" style="display:inline;">
                            ...
                            <input type="hidden" name="certID" value="<?php echo $cert['certificateID']; ?>">
                            <button type="submit" style="background:none;border:none;color:var(--success);font-size:0.8rem;font-weight:600;cursor:pointer;">
        ...
        <?php else: ?>
            <tr><td colspan="7" style="text-align:center;color:var(--text-muted);padding:2.5rem;">
                ...
                <?php if ($userRole !== 'employer'): ?>
                <a href="issue_certificate.php" style="color:var(--primary-light);font-weight:600;">Issue the first one →</a>
                <?php endif; ?>
            </td></tr>
        <?php endif; ?>
        </tbody>
