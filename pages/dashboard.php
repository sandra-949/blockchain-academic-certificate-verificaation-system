...
if ($userRole === 'admin') {
    ...
    $totalCerts = $conn->query("SELECT COUNT(*) as c FROM certificates WHERE issuedBy = $userId")->fetch_assoc()['c'];
    $validCerts = $conn->query("SELECT COUNT(*) as c FROM certificates WHERE issuedBy = $userId AND status='valid'")->fetch_assoc()['c'];
    $revokedCerts = $conn->query("SELECT COUNT(*) as c FROM certificates WHERE issuedBy = $userId AND status='revoked'")->fetch_assoc()['c'];
    $totalVerifs = $conn->query("SELECT COUNT(*) as c FROM transactions t JOIN certificates c ON t.certificateID = c.certificateID WHERE c.issuedBy = $userId AND t.transactionType='verified'")->fetch_assoc()['c'];
    $totalUsers = $conn->query("SELECT COUNT(*) as c FROM users")->fetch_assoc()['c'];

    // Recent certificates (only this institution's)
    $recentCerts = $conn->query("
        SELECT c.certificateID, c.studentName, c.program, c.dateIssued, c.status, c.hashValue, u.fullName as issuedByName
        FROM certificates c
        JOIN users u ON c.issuedBy = u.userID
        WHERE c.issuedBy = $userId
        ORDER BY c.createdAt DESC LIMIT 5
    ");

    // Recent transactions (only this institution's certificates)
    $recentTrans = $conn->query("
        SELECT t.*, c.studentName, c.program, u.fullName as verifierName
        FROM transactions t
        LEFT JOIN certificates c ON t.certificateID = c.certificateID
    ...
    <div class="col-lg-7">
        <div class="page-card">
            <div class="card-header-cv">
                ...
                <a href="certificates.php" class="btn-accent"
                    style="font-size:0.82rem;padding:0.4rem 0.9rem;border-radius:6px;">View All</a>
            </div>
                            ...
                            <?php while ($cert = $recentCerts->fetch_assoc()): ?>
                                <tr>
                                    <td><strong><?php echo htmlspecialchars($cert['studentName']); ?></strong><br>
                                        <span
                                            style="font-size:0.75rem;color:var(--text-muted);"><?php echo htmlspecialchars($cert['issuedByName']); ?></span>
                                    </td>
        ...
        <div class="page-card">
            ...
            <?php if ($recentTrans->num_rows > 0): ?>
                <?php while ($t = $recentTrans->fetch_assoc()): ?>
                    <div
                        style="display:flex;align-items:flex-start;gap:10px;padding:0.65rem 0;border-bottom:1px solid var(--border);">
                        <div
                            style="width:34px;height:34px;border-radius:50%;background:<?php echo $t['transactionType'] === 'issued' ? '#eff6ff' : ($t['verificationStatus'] === 'valid' ? '#edfaf3' : '#fff0f0'); ?>;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:0.85rem;">
                        ...
                        <div style="flex:1;min-width:0;">
                            <div
                                style="font-size:0.88rem;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                                <?php echo htmlspecialchars($t['studentName'] ?? 'Unknown'); ?>
                            </div>
                            <div style="font-size:0.78rem;color:var(--text-muted);">
                                <?php echo ucfirst($t['transactionType']); ?> &bull;
                                <?php echo date('d M Y H:i', strtotime($t['timestamp'])); ?>
                            </div>
                        </div>
                    </div>
                <?php endwhile; ?>
